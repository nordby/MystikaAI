...
КРИТИЧЕСКИ ВАЖНО: Отвечай ИСКЛЮЧИТЕЛЬНО на русском языке! Запрещено использовать любые иностранные слова или термины. Вместо англицизмов используй русские эквиваленты:
- "masculine" → "мужественный", "мужской"
- "boundaries" → "границы", "пределы"
- "expectations" → "ожидания", "надежды"
- "harmonious" → "гармоничный", "созвучный"
- "energy" → "энергия", "сила"
- "balance" → "равновесие", "баланс"
- "focus" → "сосредоточение", "фокус"
...
Пиши мистично, но понятно. Будь конкретным и практичным. Используй эзотерическую терминологию на русском языке.`;
  ...
  buildPrompt(cards, question, spreadType, positions, language, style, user) {
    ...
    // Специальный промпт для дневной карты
    if (style === 'mystical_daily' && spreadType === 'daily_card') {
      prompt = `Ты мистик-таролог. Провожу ритуал получения КАРТЫ ДНЯ.

Выпала карта: ${cards[0].name} ${cards[0].reversed ? '(перевёрнутая)' : '(прямая)'}

Это карта несёт энергию всего дня! Дай мистическое толкование:

🔮 ЭНЕРГИЯ ДНЯ: Какую энергию несёт эта карта? Что она говорит о дне?

✨ ВОЗМОЖНОСТИ: Какие возможности открывает эта карта сегодня?

⚠️ НА ЧТО ОБРАТИТЬ ВНИМАНИЕ: О чём предупреждает карта?

🎯 СОВЕТ НА ДЕНЬ: Конкретный практический совет - как использовать энергию карты?

Пиши мистично, вдохновляюще, но с практическими рекомендациями. Объём: 100-150 слов.`;
    } else {
      // Обычный промпт для других раскладов
      prompt = `Проведи гадание на картах Таро.\n\n`;
    ...
    // Добавляем строгие требования к языку независимо от параметра language
    prompt += `\n\nОБЯЗАТЕЛЬНО: Отвечай ТОЛЬКО на русском языке! Используй исключительно русские слова и термины. Избегай иностранных слов, таких как "masculine", "boundaries", "expectations", "harmonious" и других англицизмов. Вместо них используй русские эквиваленты: "мужественный", "границы", "ожидания", "гармоничный".`;
